...
function GetItems(setItems) {
    fetch('http://edu-testback-end.com/chapter/AdminGetAllChapters.php', {
        method: "GET",
    })
...
function AddItem() {
    fetch('http://edu-testback-end.com/chapter/AdminAddChapter.php', {
        method: "GET",
    })
        ...
        .then(response => {
            if (response['error'])
                throw (response['error']);
            document.location.href = '/admin/chapter/' + response;
        })
}
    ...
    const columns = React.useMemo(
        () => [
            ...
            },
            {
                ...
                Cell: ({ row }) => {
                    return (<>
                        <a href={"/admin/chapter/" + row.original.idChapter} className="go"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" viewBox="0 0 16 16">
