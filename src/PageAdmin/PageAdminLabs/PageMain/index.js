...
function AddItem() {
    ...
    fetch(config.backHost + 'labs/AdminAddLab.php', {
        ...
        .then(response => {
            if (response['error'])
                throw (response['error']);
            document.location.href = config.frontHost + 'admin/lab/' + response;
        })
}
    ...
    const columns = React.useMemo(
        () => [
            ...
            },
            {
                ...
                Cell: ({ row }) => {
                    return (<>
                        <a href={config.frontHost + "admin/lab/" + row.original.idLab} className="go"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" viewBox="0 0 16 16">
